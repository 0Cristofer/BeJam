...
#region UNITY_EDITOR Section
    ...
    private string GetSceneNameFromAsset()
    {
        ...
        string _sceneName = AssetDatabase.GetAssetPath(sceneAsset);

        if (string.IsNullOrEmpty(_sceneName))
            return string.Empty;

        // Strip the everything before and after the name so no path and extension
        _sceneName = _sceneName.Remove(0, _sceneName.LastIndexOf("/") + 1);
        _sceneName = _sceneName.Remove(_sceneName.LastIndexOf("."));
...
public class SceneReferencePropertyDrawer : PropertyDrawer
{
    ...
    private void DrawSceneInfoGUI(Rect position, BuildUtils.BuildScene buildScene, int sceneControlID)
    {
        ...
        {
            iconContent = EditorGUIUtility.IconContent("d_winbtn_graph_max_h");
        ...
        {
            iconContent = EditorGUIUtility.IconContent("d_winbtn_graph_min_h");
            labelContent.text = "BuildIndex: " + buildScene.buildIndex;
        ...
        using (new EditorGUI.DisabledScope(readOnly))
        {
            ...
            iconRect.width = iconContent != null ? (iconContent.image.width + padSize) : 1.0f;
